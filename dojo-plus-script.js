...
function renderSchoolCard(s) {
  return `<div class="card listing-school-card" data-city="${s.ciudad?.toLowerCase()}" data-disciplines="${(s.disciplinas||[]).join(' ').toLowerCase()}" onclick="goTo('school')">
    ...
    <div class="school-card-body">
      ...
      <div class="school-card-foot">
        ...
        <a class="wa-icon" href="https://wa.me/${s.whatsapp}" target="_blank" onclick="event.stopPropagation()">📲</a>
      </div>
    </div></div>`;
}

function renderTrainerCard(t) {
  return `<div class="card trainer-card" data-city="${t.ciudad?.toLowerCase()}" data-disciplines="${(t.disciplinas||[]).join(' ').toLowerCase()}" onclick="goTo('trainer')">
    <div class="trainer-photo-wrap"><div class="trainer-photo" style="font-size:28px">🥋</div></div>
