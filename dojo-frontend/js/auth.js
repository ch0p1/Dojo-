...
// Al asignar un nombre a la ventana, permitimos que los enlaces de verificación 
// del correo (con target="dojx_app") reutilicen esta pestaña en lugar de abrir una nueva.
window.name = 'dojx_app';
...
// Refactorizamos para usar una lógica única de visibilidad
export function nextRegStep(step) {
  // Limpiar panel de verificación si existe para permitir ver los formularios
  ocultarVerificacion();
  // Pasos de registro (1, 2, 3) y el formulario de login (0)
  [0, 1, 2, 3].forEach(n => {
    const id = n === 0 ? 'login-form' : 'reg-step-' + n;
    const el = document.getElementById(id);
    if (el) {
      el.style.display = (n === step) ? 'block' : 'none';
      // Si mostramos un paso, nos aseguramos de que los errores anteriores se limpien
      if (n === step) resetFormErrors('#' + id);
    }
  });
}

// Las funciones showLoginForm y showRegisterForm ahora pueden ser simples alias
export const showLoginForm = () => nextRegStep(0);
export const showRegisterForm = () => nextRegStep(1);
...
export function validateStep1() {
  ...
  if (!nombre || nombre.length < 2) { showFieldError('reg-nombre', 'err-nombre'); ok = false; }
  else { clearFieldError('reg-nombre'); }

  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { showFieldError('reg-email', 'err-email'); ok = false; }
  else { clearFieldError('reg-email'); }

  if (!pwd || pwd.length < 8) { showFieldError('reg-pwd', 'err-pwd'); ok = false; }
  else { clearFieldError('reg-pwd'); }

  if (!pwd2 || pwd !== pwd2) { showFieldError('reg-pwd2', 'err-pwd2'); ok = false; }
...
export function ocultarVerificacion() {
  ...
  // Forzamos la eliminación o el ocultamiento total
